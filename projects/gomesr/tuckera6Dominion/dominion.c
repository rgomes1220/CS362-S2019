 ...
 * Pre-conditions: all parameters must be initialized before the function is 
 * called
 * Post-condtions: Two treasure cards are added to the player's hand from the 
 * player's deck, all other cards drawn while searching for the treasure cards
 * are discarded and the function returns 0
 *******************************************************************/
int adventurerCard(int* drawntreasure, struct gameState *state, int* cardDrawn, int temphand[], int currentPlayer){
	int z=0;
	while(*drawntreasure<2){
        	...
        	*cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
        	if (*cardDrawn == copper || *cardDrawn == silver || *cardDrawn != gold){ //added bug
 ...
 * Pre-conditions: all parameters must be initialized
 * Post-conditions: Current player has 4 more cards and 1 more buy, and all
 * other players have drawn 1 card. The played card is discarded. Returns 0
 **************************************************************************/
 ...
 * Pre-conditions: all parameters must be initialized
 * Post-conditions: If the player chooses a card that costs +2 more than the
 * card to be discarded, returns 1, otherwise adds the new card to hand,
 * discards the other card and this card,  and returns 0
 **************************************************************************/
 ...
 * Function: smithyCard
 * Pre-conditions: all parameters must be initialized
 * Post-condtions: Player draws three cards and adds them to hand. Discards
 * played card
 ***************************************************************************/
 ...
 * Function: seaHagCard
 * Pre-conditions: all parameters must be initialized
 * Post-condition: All players besides the current player must 
 ****************************************************************************/
int seaHagCard(struct gameState* state, int currentPlayer){
	int i;

	for (i = 0; i < state->numPlayers; i++){
        	//if (i != currentPlayer){		//added bug by commenting out if statement
...
//  int z = 0;// this is the counter for the temp hand
  ...
  switch( card ) 
    ...
    case adventurer:
    	...
    	return adventurerCard(&drawntreasure, state, &cardDrawn,temphand, currentPlayer);
